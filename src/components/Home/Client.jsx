...
const Client = () => {
    ...
    return(
        <main className={`w-full ${isAboutPage ? "bg-thin-white": "bg-white" }`}>
            <section className="flex flex-col py-30 gap-15">
            {
                isAboutPage && (
                    <article className="flex flex-col items-center text-center justify-center gap-8 px-15 font-montserrat">
                        <h2 className="font-bold text-[40px] leading-13 text-text-color">Big Companies Are Here</h2>
                        <p className="font-normal text-sm leading-5 text-second-text-color">Problems trying to resolve the conflict between the two major realms of Classical physics: Newtonian mechanics </p>
                    </article>
                )
            }
