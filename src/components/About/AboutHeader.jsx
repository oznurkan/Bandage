              ...
              {aboutSection && (
                <section className="flex flex-col w-full h-full pt-40 pb-10 xl:flex-row xl:pt-10">
                  <article className="flex flex-col items-center justify-center text-center gap-10 px-19 font-montserrat lg:px-80 xl:flex-1 xl:items-start xl:text-start xl:px-37">
                    <h5 className="hidden font-bold text-base leading-6 text-text-color xl:flex">{aboutSection.aboutHeader.subtitle}</h5>
                    ...
                    <h4 className="font-normal text-xl leading-8 text-second-text-color">
                      {aboutSection.aboutHeader.paragraph?.split(",").map((text, index) => (
                            <span key={index}>
                              {text.trim()}
                              {index === 0 && ","}
                    ...
                    <button className="text-sm leading-6 text-white py-4 px-10 rounded-md gap-2.5 bg-primary-color">{aboutSection.aboutHeader.buttonText}</button>
